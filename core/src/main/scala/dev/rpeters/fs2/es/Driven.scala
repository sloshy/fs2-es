...
trait Driven[E, A] {
  ...
  /** Apply an event to a state that may or may not exist.
    *
    * If a state does not exist and it is created, that indicates it is said to have been initialized by some starting
    * event. If a state does exist but `None` is returned after applying this function, that state is said to be
    * deleted.
    *
    * @param optA
    *   A possibly-existing state.
    * @param e
    *   An event to apply to your state.
    * @return
    *   A new state, or no state value, depending on the implementation.
    */
...
object Driven {
  ...
  /** Create a `Driven` instance for applying events to possibly-existing state.
    *
    * If the supplied partial function is not defined for your inputs to `handleEvent` when called, the default behavior
    * is to return the input value.
    *
    * @param f
    *   A function to apply events to state.
    * @return
    *   An instance of `Driven` for your state type.
    */
  ...
  /** Create a `Driven` instance derived from your state's `Monoid` instance.
    *
    * In the event that your state is `None`, it is initialized as your type's "empty value", such as 0 for integers or
    * the empty string.
    */
