...
trait EventStateLogOps[F[_], S[_, _], E, A] {
  ...
  /** Attaches a `FiniteEventLog` as an `EventLogSink`, but it also applies the entire event log to the current state
    * first before returning it. Result is a singleton stream containing your new state.
    */
